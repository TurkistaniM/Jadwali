...
export function calculateElapsedAbsenceStats(
  ...
  attendances.forEach(att => {
    ...
    if (att.status === 'تم إلغاء الدرس' || att.status === 'cancelled') {
      cancelledCount++;
      // مستثناة تماماً من إجمالي المحاضرات المحسوبة
      return;
    }

    if (att.status === 'حاضر' || att.status === 'present') {
      presentCount++;
    } else if (att.status === 'غائب' || att.status === 'absent') {
      absentCount++;
    } else if (att.status === 'متأخر' || att.status === 'late') {
      lateCount++;
    }
  });
